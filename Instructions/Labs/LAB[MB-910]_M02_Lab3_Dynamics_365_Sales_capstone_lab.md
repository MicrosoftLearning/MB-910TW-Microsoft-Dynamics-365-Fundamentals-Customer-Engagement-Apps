...
10. 請注意在記錄頂端的**潛在客戶變商機**商務程序流程。按一下**授與資格階段**以選取該潛在客戶。完成階段，如下所示：
...
11. 請按一下階段視窗上的 **X** 來關閉視窗。 

12. 請移至畫面中央的**時間表**並按一下**加號圖示**以新增活動。 
...
2. 請選取**我待完成的商機**檢視表旁的下拉式箭頭，並從顯示的功能表中選取**開啟商機**。

3. 在命令列上，請選取顯示圖表。請注意，[最佳客戶] 圖表會依照商機資料表來顯示。 

4. 請選取 [最佳客戶] 旁的下拉式箭頭，從顯示的功能表中選取**準銷售案源**。

5. 請選取漏斗圖的 [授與資格] 部分。請注意，商機清單將改為顯示處於授與資格階段的商機。 

6. 請按一下圖表空白處的任意位置，以再次顯示所有待完成的商機。 
...
12. 請移至畫面中央的**記錄時間表**，並按一下**加號圖示**以新增活動。 
...
17. 請按一下階段視窗上的 **X** 以將其關閉，使您可以繼續工作。 

18. 在**利害關係人**子格中，注意到 **Piper** 已定義為利害關係人。 

19. 在 [銷售團隊] 子格中，選取**垂直省略符號**。從顯示的功能表中選取**新連線**。 

20. 在**搜尋**欄位中，請輸入文字**系統**並選取**系統管理員**記錄。一旦完成後，按一下**新增**按鈕。系統管理員現在應會出現在銷售團隊中。如果沒有，請選取命令列上的**重新整理**按鈕。 
...
25. 請確保已選取您剛剛建立的 Coho Security 記錄，接著選取**新增**按鈕。 

26. 按一下以選取**潛在客戶變商機**商務程序流程上的**開發**階段，將**識別利害關係人**和**識別競爭者**步驟設為**已完成**。 

27. 按**下一個階段**按鈕以進入**提案**階段。

28. 在**提案**階段中，將**識別銷售團隊**標記為**已完成**。

29. 請按一下提案階段上的 **X** 來關閉階段視窗。 

30. 在商機記錄上，選取**報價**索引標籤。 

31. 在 [報價] 子格上，按一下**新增報價**按鈕。
...
1. 在**命令列**上，選取**啟用報價**按鈕以啟用報價。 

2. 您現在需要選取要附加到商機的價目表。  在左側窗格的**價目表**下，選取搜尋圖示，然後從選項中選取**預設價目表**。

3. 現在報價已建立，讓我們來更新商機記錄以反映新的資料。在 [報價] 記錄的**銷售資訊**區段下的**商機**欄位中，選取**尋找安全設備** -**「您的姓名縮寫」**商機。該商機記錄應會在畫面中開啟。 

3. 在 [商機] 記錄上，請選取**提案**階段。 

4. 請將**開發提案**、**完成內部審查**和**提出提案**標記為**已完成**，然後按一下**下一個階段**按鈕以進入**關閉**階段。 
